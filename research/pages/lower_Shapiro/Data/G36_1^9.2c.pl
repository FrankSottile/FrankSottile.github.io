...
#
#  schubert took: 
#  4.950 to compute 10 instances with number size 100
#  13.900 to compute 10 instances with size number 1000 
#
...
for (my $outer=1; $outer<=$number_files; $outer++)
{
  ...
  for (my $i=1; $i<=$number_per_file; $i++){
    printf SING "I = \n";
    foreach $pols (get_equations(4,2,'G36_1_inf')){
      printf SING " $pols+";
    }
    printf SING "0;\n";
    printf SING "I = std(I);\n";
