...
my $prblmname = "G27_2^2_1^6.3c";
my $number_solutions = 19;
my $number_files = 20;
my $number_per_file = 50;

my $size = 500;
#
#  42 to compute 100 instances with number size 500
#
#
...
for (my $outer=1; $outer<=$number_files; $outer++)
{
  ...
  for (my $i=1; $i<=$number_per_file; $i++){
    printf SING "I = \n";
    foreach $pols (get_equations(0,1,'G27_2')){
      printf SING " $pols+";
    }
    foreach $pols (get_equations(2,2,'G27_1')){
      printf SING " $pols+";
    }
    printf SING "0;\n";
    printf SING "I = std(I);\n";
