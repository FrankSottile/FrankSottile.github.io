...
#
#			x2^2 y1^5 = 11 in Gr(3,6)
#
#			(lambda = 2 at infinity)
#
...
my $prblmname = "G36_2^2_1^5.0c2c";
my $number_solutions = 11;
my $number_files = 100;
my $number_per_file = 20;

my $size = 500;
#
#   to compute 100 instances with size number 500 
#
...
for (my $outer=1; $outer<=$number_files; $outer++)
{
  ...
  #printf SING  "ring R = 32003, (a,b,c,d,e,f,i), dp;\n";
  printf SING  "ring R = 0, (a,b,c,d,e,f,i), dp;\n";
  printf SING  "ideal I;\n";
  ...
  for (my $i=1; $i<=$number_per_file; $i++){
    printf SING "I = \n";
    foreach $pols (get_equations(1,0,'G36_2_inf')){
	printf SING " $pols+";
    }
    foreach $pols (get_equations(1,2,'G36_1_2inf')){
      printf SING " $pols+";
    }
    printf SING "0;\n";
    printf SING "I = std(I);\n";
