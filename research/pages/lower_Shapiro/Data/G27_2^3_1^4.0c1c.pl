...
my $prblmname = "G27_2^3_1^4.0c1c";
my $number_solutions = 13;
my $number_files = 50;
my $number_per_file = 200;

my $size = 1000;
#
#  .57 to compute 10 instances with size = 100
#  .87 to compute 10 instances with size = 1000
#
...
for (my $outer=1; $outer<=$number_files; $outer++)
{
  ...
  for (my $i=1; $i<=$number_per_file; $i++){
    printf SING "I = \n";
    foreach $pols (get_equations(3,0,'G27_2')){
      printf SING " $pols+";
    }
    foreach $pols (get_equations(2,1,'G27_1')){
      printf SING " $pols+";
    }
    printf SING "0;\n";
    printf SING "I = std(I);\n";
