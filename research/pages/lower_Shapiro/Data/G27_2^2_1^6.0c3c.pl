...
my $prblmname = "G27_2^2_1^6_2inf.3c";
my $number_solutions = 19;
my $number_files = 500;
my $number_per_file = 200;

my $size = 500;
#
#  18 to compute 100 files with number size 500
#
#
...
for (my $outer=1; $outer<=$number_files; $outer++)
{
  ...
  #printf SING  "ring R = 32003, (a,b,c,d,e,h,i,j), dp;\n";
  printf SING  "ring R = 0, (a,b,c,d,e,h,i,j), dp;\n";
  printf SING  "ideal I;\n";
  ...
  for (my $i=1; $i<=$number_per_file; $i++){
    printf SING "I = \n";
    foreach $pols (get_equations(1,0,'G27_2_inf')){
      printf SING " $pols+";
    }
    foreach $pols (get_equations(0,3,'G27_1_2inf')){
      printf SING " $pols+";
    }
    printf SING "0;\n";
    printf SING "I = std(I);\n";
