...
#
#  schubert took: 
#  12.950 to compute 10 instances with number size 300
#  20.750 to compute 10 instances with size number 500 
#
...
for (my $outer=1; $outer<=$number_files; $outer++)
{
  ...
  for (my $i=1; $i<=$number_per_file; $i++){
    printf SING "I = \n";
    foreach $pols (get_equations(0,4,'G36_1_inf')){
      printf SING " $pols+";
    }
    printf SING "0;\n";
    printf SING "I = std(I);\n";
